...
export const SKILL_TREES = {
    intuition: {
        name: 'Street Smart',
        skills: {
            lockpick: { name: 'Lockpick', tier: 1, passive: true, effects: { safehouseEntrySpeed: 0.5 }, bonus: 'Can unlock' },
            street_sense: { name: 'Street Sense', tier: 1, passive: true, effects: { detectHidden: true }, bonus: 'Reveals' },
            sneak: { name: 'Sneak', tier: 2, passive: true, effects: { sneakHeatReduction: 1.0 }, bonus: 'Stealth' },
            intimidate: { name: 'Intimidate', tier: 2, active: true, cooldown: 30, effects: { intimidateChance: 0.5 }, bonus: '50% flee' },
            shadow_walk: { name: 'Shadow Walk', tier: 3, active: true, cooldown: 60, effects: { invisibleDuration: 5 }, bonus: '5s invis' },
            deadly_precision: { name: 'Deadly Precision', tier: 3, passive: true, effects: { critChance: 0.25, critDamage: 0.5 }, bonus: '+25% crit' }
        }
    },
    ability: {
        name: 'Enforcer',
        skills: {
            iron_fist: { name: 'Iron Fist', tier: 1, passive: true, effects: { damageBonus: 0.25 }, bonus: '+25% dmg' },
            toughness: { name: 'Toughness', tier: 1, passive: true, effects: { hpBonus: 25, damageReduction: 0.1 }, bonus: '+25 HP' },
            power_strike: { name: 'Power Strike', tier: 2, active: true, cooldown: 15, effects: { powerStrikeDamage: 2.0 }, bonus: '2x dmg' },
            last_stand: { name: 'Last Stand', tier: 2, passive: true, effects: { lastStandThreshold: 0.25 }, bonus: '+50% at low HP' },
            berserk: { name: 'Berserk', tier: 3, active: true, cooldown: 90, effects: { berserkDuration: 8, berserkDamage: 2.0 }, bonus: '8s 2x' },
            unbreakable: { name: 'Unbreakable', tier: 3, passive: true, effects: { deathSave: 1 }, bonus: 'Survive' }
        }
    },
    weapons: {
        name: 'Combat Master',
        skills: {
            dual_wield: { name: 'Dual Wield', tier: 1, passive: true, effects: { secondWeaponSlot: true }, bonus: '2nd slot' },
            quick_draw: { name: 'Quick Draw', tier: 2, passive: true, effects: { attackSpeedBonus: 0.2 }, bonus: '+20% spd' },
            automatic_weapons: { name: 'Auto Weapons', tier: 3, passive: true, effects: { automaticWeapons: true }, bonus: 'Auto guns' },
            dead_eye: { name: 'Dead Eye', tier: 3, passive: true, effects: { damageBonus: 0.3, critChance: 0.1 }, bonus: '+30% dmg' }
        }
    },
    luck: {
        name: 'Con Artist',
        skills: {
            fast_talk: { name: 'Fast Talk', tier: 1, passive: true, effects: { vendorDiscount: 0.15 }, bonus: '15% off' },
            lucky_break: { name: 'Lucky Break', tier: 1, passive: true, effects: { luckBonus: 0.2 }, bonus: '+20% luck' },
            card_shark: { name: 'Card Shark', tier: 2, passive: true, effects: { cardGameWinRate: 0.6 }, bonus: '60% win' },
            escape_artist: { name: 'Escape Artist', tier: 2, passive: true, effects: { escapeChance: 0.75 }, bonus: '75% escape' },
            fortune: { name: 'Fortune', tier: 3, passive: true, effects: { xpBonus: 0.2, allLuck: 0.1 }, bonus: '+20% XP' },
            game_of_chance: { name: 'Game of Chance', tier: 3, active: true, cooldown: 120, effects: { doubleOrNothing: true }, bonus: '50/50' }
        }
    }
};
...
export default class SkillTree {
    ...
    canLearnSkill(skillKey) {
        ...
        if (!skill) return { canLearn: false, reason: 'Not found' };
        if (playerState.unlockedSkills.includes(skillKey)) return { canLearn: false, reason: 'Learned' };
    ...
    hasSkill(skillKey) { return this.scene.playerState.unlockedSkills?.includes(skillKey); }
    ...
    activateSkill(skillKey, combatScene) {
        ...
        const { skill } = this.findSkillDefinition(skillKey);
        if (!skill?.active) return { success: false, reason: 'Not active' };
        const lastUsed = playerState.skillCooldowns?.[skillKey] || 0;
        const now = this.scene.time.now / 1000;
        if (skill.cooldown && now - lastUsed < skill.cooldown) return { success: false, reason: 'Cooldown' };
        if (!playerState.skillCooldowns) playerState.skillCooldowns = {};
        playerState.skillCooldowns[skillKey] = now;
        
        switch (skillKey) {
            case 'intimidate':
                if (Math.random() < 0.5) { combatScene.showDamageText('ENEMY FLED!', combatScene.scale.width/2, 300, 0x00ff00); combatScene.enemyHP = 0; combatScene.enemyFled = true; }
                else combatScene.showDamageText('Intimidate failed!', combatScene.scale.width/2, 300, 0xffaa00);
                break;
            case 'shadow_walk':
            ...
            case 'berserk':
                ...
                break;
            case 'game_of_chance':
                if (Math.random() < 0.5) { const luckDmg = combatScene.getPlayerDamage() * 2; combatScene.enemyHP = Math.max(0, combatScene.enemyHP - luckDmg); combatScene.showDamageText('LUCKY! -' + luckDmg, combatScene.scale.width/2, 220, 0xffd700); }
                else { const badLuck = Math.floor(combatScene.enemy.damage * 0.5); combatScene.playerHP = Math.max(0, combatScene.playerHP - badLuck); combatScene.showDamageText('BAD LUCK! +' + badLuck, combatScene.scale.width/2, 480, 0xff0000); }
                break;
        }
...
class SkillTreeUI {
    ...
    open() {
        ...
        this.container.add(this.scene.add.rectangle(width/2, height/2, 1000, 700, 0x1a1a2e).setStrokeStyle(4, 0x4a4a6a));
        this.container.add(this.scene.add.text(width/2, 50, 'SKILLS', { fontFamily: 'Press Start 2P', fontSize: '28px', color: '#ffd700' }).setOrigin(0.5));
    ...
    createCard(x, y, skill) {
        ...
        const card = this.scene.add.rectangle(x, y, 260, 110, color).setStrokeStyle(2, stroke);
        this.container.add(card);
        this.container.add(this.scene.add.text(x, y - 35, skill.name, { fontFamily: 'Press Start 2P', fontSize: '12px', color: skill.learned ? '#00ff00' : '#fff' }).setOrigin(0.5));
        this.container.add(this.scene.add.text(x, y, skill.bonus || '', { fontFamily: 'Press Start 2P', fontSize: '9px', color: '#aaa', wordWrap: { width: 240 } }).setOrigin(0.5));
        
        if (!skill.learned && skill.canLearn) {
            const learnBtn = this.scene.add.rectangle(x, y + 40, 100, 25, 0x4a4a2a).setStrokeStyle(1, 0xffd700).setInteractive({ useHandCursor: true });
            this.container.add(learnBtn);
            this.container.add(this.scene.add.text(x, y + 40, 'LEARN', { fontFamily: 'Press Start 2P', fontSize: '10px', color: '#ffd700' }).setOrigin(0.5));
            learnBtn.on('pointerup', () => { if (this.skillTreeManager.learnSkill(skill.key)) { this.close(); this.scene.time.delayedCall(100, () => this.skillTreeManager.openSkillTreeUI()); } });
        }
    }
