...
export default class RivalEncounterUI {
    ...
    checkWin(playerChoice, rivalChoice) {
        // Check for neighborhood bonus (extra dice in RPS)
        const neighborhoodBonus = this.scene.playerState.neighborhoodBonus;
        const hasExtraDice = neighborhoodBonus?.rpsExtraDice || false;
        
        if (playerChoice === rivalChoice) {
            // Tie - player wins if their stat is higher (with possible neighborhood bonus)
            const playerStat = this.scene.playerState.stats[playerChoice];
            // Neighborhood bonus: effectively adds +3 to stat for tie-breaker
